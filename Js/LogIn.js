...
function initializeLogin() {
    ...
    // Demo users for testing (in real app, this would be handled by backend)
    const demoUsers = [
        { username: 'admin', password: 'admin123', name: 'Administrator' },
        { username: 'student', password: 'student123', name: 'Student User' },
        { username: 'teacher', password: 'teacher123', name: 'Teacher User' },
        { username: 'nour', password: 'nour123', name: 'Nour' }
    ];
    ...
    function validateField(fieldName) {
        ...
        if (rules.minLength && value.length < rules.minLength) {
            showFieldError(input, `${fieldName.charAt(0).toUpperCase() + fieldName.slice(1)} must be at least ${rules.minLength} characters`);
            return false;
        }

        // Check maximum length
        if (rules.maxLength && value.length > rules.maxLength) {
            showFieldError(input, `${fieldName.charAt(0).toUpperCase() + fieldName.slice(1)} must not exceed ${rules.maxLength} characters`);
            return false;
        }
    ...
    // Authenticate user (simulate backend authentication)
    ...
    function handleForgotPassword(event) {
        ...
        const username = usernameInput.value.trim();
        if (!username) {
            showInfo('Please enter your username first, then click "Forgot password?"');
            usernameInput.focus();
            return;
        }

        // Simulate password reset
        showInfo(`Password reset instructions have been sent to the email associated with "${username}".`);
    }

    // Handle register link click
    function handleRegisterClick(event) {
        event.preventDefault();
        showInfo('Registration feature coming soon! For now, you can use these demo accounts:\n\n' +
                 '• Username: admin, Password: admin123\n' +
                 '• Username: student, Password: student123\n' +
                 '• Username: teacher, Password: teacher123\n' +
                 '• Username: nour, Password: nour123');
    }
